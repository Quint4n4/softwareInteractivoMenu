      ...
      <div className="v-mthumb">
        ...
        {sold ? (
          ...
          <span className="v-mtag">
            {it.etiqueta === "Favorito" ? <Icon name="star" size={10} fill="var(--v-accent)" stroke="none" /> : <Icon name="leaf" size={10} />}
...
/* ---------------- Home ---------------- */
function HomeScreen({ data, cats, goMenu, ui, lang, idiomas, onLang, vista, hasTabs, onVista }: {
  data: PublicMenu; cats: PubCategoria[];
  goMenu: (catId?: number) => void;
    ...
    <div>
      <div className="v-toprow">
        <div className="v-brand">
          {data.tema.logo ? (
            <img className="v-brand__logo" src={mediaUrl(data.tema.logo) ?? ""} alt={data.negocio.nombre} />
          ) : (
            <span className="v-brand__logo v-brand__logo--ph"><Icon name="coffee" size={20} /></span>
          )}
          <div className="v-brand__txt">
            <div className="v-brand__name">{data.negocio.nombre}</div>
            <div className="v-brand__sub"><Icon name="pin" size={11} /> {ui.menuDigital}</div>
          </div>
        </div>
      ...
      <div className="v-home-head">
        <div className="v-home-title">{ui.chooseTitle}</div>
        <div className="v-home-sub">{ui.tapCategory}</div>
      </div>
      <div className="v-catgrid">
        {cats.map((c) => {
          const catImg = c.items.find((it) => it.imagen)?.imagen ?? null;
          return (
            <button key={c.id} className="v-catcard" onClick={() => goMenu(c.id)}>
              <div className="v-cat-thumb">
                {catImg ? <img src={mediaUrl(catImg) ?? ""} alt={c.nombre} /> : <Icon name="image" size={26} />}
              </div>
              <div>
...
/* ---------------- Menu ---------------- */
function MenuScreen({ cats, active, qtyOf, onAdd, onDec, onOpen, goHome, ui, lang, idiomas, onLang, vista }: {
  cats: PubCategoria[]; active: number | null;
  qtyOf: (id: number) => number; onAdd: (id: number) => void; onDec: (id: number) => void;
  onOpen: (it: PubItem) => void; goHome: () => void;
  ui: UIStrings; lang: string; idiomas: string[]; onLang: (l: string) => void;
  vista: "menu" | "tienda";
}) {
  ...
  return (
    <div>
      ...
      <section className="v-section">
        <div className="v-sec-head"><span className="v-sec-count">{cat.items.length} {ui.options}</span></div>
        <div className={vista === "tienda" ? "v-grid" : "v-mgrid"}>
          {cat.items.map((it) => (vista === "tienda"
...
function CheckoutScreen({ lines, subtotal, guest, busy, onAdd, onDec, onRemove, goMenu, onConfirm, ui }: {
  ...
  goMenu: () => void; onConfirm: (f: { nota: string; propina: number; metodo: string; telefono: string }) => void; ui: UIStrings;
}) {
...
function TrackingScreen({ order, onReorder, onTicket, ui }: { order: Pedido; onReorder: () => void; onTicket: () => void; ui: UIStrings }) {
  ...
  return (
    <div>
      ...
      <div style={{ padding: "4px 18px 0" }}>
        ...
        <button className="v-btn" onClick={onTicket} style={{ marginBottom: 10 }}><Icon name="receipt" size={17} /> {ui.ticketBtn}</button>
        <button className="v-btn v-btn--soft" onClick={onReorder} style={{ marginBottom: 8 }}>{ui.orderAgain}</button>
      </div>
    </div>
  );
}
...
/* ---------------- Ticket (WhatsApp) ---------------- */
function TicketSheet({ order, negocio, ui, onClose }: { order: Pedido; negocio: string; ui: UIStrings; onClose: () => void }) {
  const tipo = order.tipo === "mesa" ? `${ui.table} ${order.mesa_texto || "—"}` : ui.takeout;
  return (
    <div className="v-sheet-overlay" onClick={onClose}>
      <div className="v-sheet" onClick={(e) => e.stopPropagation()}>
        <div className="v-sheet-grip" />
        <div className="v-ticket">
          <div className="v-ticket__biz">{negocio}</div>
          <div className="v-ticket__meta">{order.numero} · {tipo} · {order.nombre_cliente}</div>
          <div className="v-ticket__rule" />
          {order.lineas.map((l) => (
            <div key={l.id} className="v-ticket__row">
              <span>{l.cantidad}× {l.nombre}</span>
              <span>{money(Number(l.precio_unitario) * l.cantidad)}</span>
            </div>
          ))}
          <div className="v-ticket__rule" />
          <div className="v-ticket__row"><span>{ui.subtotal}</span><span>{money(Number(order.subtotal))}</span></div>
          <div className="v-ticket__row"><span>{ui.tip}</span><span>{money(Number(order.propina))}</span></div>
          <div className="v-ticket__row v-ticket__total"><span>{ui.total}</span><span>{money(Number(order.total))}</span></div>
          <div className="v-ticket__row"><span>{ui.payment}</span><span>{order.metodo_pago_label}</span></div>
          <div className="v-ticket__thanks">{ui.thanks}</div>
        </div>
        <p className="v-co-note" style={{ marginTop: 16 }}>{order.telefono ? ui.willSendWhatsapp : ui.ticketNoPhone}</p>
      </div>
    </div>
  );
}
...
/* ---------------- Tab bar ---------------- */
function TabBar({ screen, go, cartCount, ui }: { screen: Screen; go: (s: Screen) => void; cartCount: number; ui: UIStrings }) {
  const tabs: { id: Screen; label: string; icon: string }[] = [
    { id: "home", label: ui.tabs.home, icon: "home" },
    { id: "cart", label: ui.tabs.cart, icon: "clipboard" },
    { id: "pedido", label: ui.tabs.order, icon: "receipt" },
  ];
  const activeTab: Screen = screen === "menu" ? "home" : screen;
  return (
...
export default function Vitrina() {
  ...
  const [cart, setCart] = useState<Cart>(() => {
    try { return JSON.parse(localStorage.getItem(`vd_cart_${slug}`) || "{}") as Cart; } catch { return {}; }
  });
  ...
  useEffect(() => {
    localStorage.setItem(`vd_cart_${slug}`, JSON.stringify(cart));
  }, [cart, slug]);
  ...
  async function confirmar(f: { nota: string; propina: number; metodo: string; telefono: string }) {
    if (!guest) return;
    setSending(true);
    try {
  ...
  const rootStyle = { "--v-accent": accent } as CSSProperties;
  ...
  return (
    <div className="vitrina" style={rootStyle}>
      <div className={"v-app" + (cartCount > 0 && screen !== "cart" ? " v-app--fab" : "")}>
        ...
        {screen === "menu" && (
          <MenuScreen
            cats={cats} active={active}
            qtyOf={qtyOf} onAdd={addItem} onDec={decItem} onOpen={setOpenItem}
            goHome={() => { setScreen("home"); window.scrollTo({ top: 0 }); }} ui={ui}
            lang={activeLang} idiomas={idiomas} onLang={chooseLang} vista={vista}
          />
        )}
        {screen === "cart" && guest && (
          <CheckoutScreen
      ...
      {ticketOpen && order && (
        <TicketSheet order={order} negocio={data.negocio.nombre} ui={ui} onClose={() => setTicketOpen(false)} />
      )}
    </div>
  );
}
